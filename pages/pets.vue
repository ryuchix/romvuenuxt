      ...
      <div class="filters d-flex flex-row justify-center justify-space-around">
        ...
        <div class="all-types">
          <v-menu max-height="300" bottom offset-y :close-on-click="closeOnClick">
            ...
            <v-list tile dense min-width="100">
              <v-list-item link v-for="(type, index) in unlocks" :key="index"
                :class="$route.query.type == type ? '_active' : ''"
                @click="$router.push({name: 'pets', query: query({unlock:type})})">
                ...
                <div class="row">
                    <div class="monsters row no-gutters">
                        <div class="col-sm-12 col-12">
                          <nuxt-link :to="{ name: 'pet-slug', params: { slug: monster.slug }}">
...
export default {
  ...
  head() {    
    ...
    let keywords_ = 'Ragnarok pets database, pets fusion, pets taming, ROM, ROM Exchange price, market finance, Ragnarok, online, RO, ragnarok mobile, ragnarok m, ragnarok eternal love, database, guide, job, quest, headgear quest, monster drops, item information, skill description, skill simulator, stat calculator, ragnarok tools, ragnarok mobile english'
    let description_ = 'Browse the Ragnarok Mobile pets database. Your ultimate guide for Ragnarok Mobile Eternal Love. Your source for Ragnarok M Monsters, Cards, Quests, Database, Headwears, Blueprints, Items, Market Prices, Exchange Price List and Stats and Skills calculator. ROM'
    return {
      title: title_,
      meta: [
        {
  ...
  data() {
    return {
      ...
      q: this.$route.query,
      fquery: this.$route.fullPath.replace("/pets", "")
    }
  },
  mounted() {
